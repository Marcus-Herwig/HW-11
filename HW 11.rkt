...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |HW 11|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;***** Environment Stuff *****
(define resolve
  (lambda (varName env)
    (cond
...
(define resolve-scope
  (lambda (varName scope)
    (cond
      ((null? scope) #f)
      ((eq? varName (caar scope)) (if(symbol?  (cadar scope)) (resolve (cadar scope)env) (cadar scope)))
...
(define construct-scope
  (lambda (list-of-2-lists env)
    (map (lambda (2-list) (list (car 2-list) (run-parsed-code
                                              (if(symbol? (cadr 2-list))
                                                 (resolve (cadr 2-list) env)
                                                 (cadr 2-list))
                                               env))) list-of-2-lists)))
...
(define burn-global
  (lambda (env)
    (if (eq? (caar env) 'global)
        '()
        (cons (car env) (burn-global (cdr env))))))
    ...
    (cond
      ...
      ((eq? (car parsed-no-code) 'let-exp)
       (let* ((new-scope (construct-scope (cadr parsed-no-code) env))
              (new-env (extend-env new-scope env)) ;(list (list (list 'a 2) (list 'b 7)) (list 'global (list 'age 21) (list 'a 7) (list 'b 5) (list 'c 23)))
